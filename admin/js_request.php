...
if(isset($_POST['activate_user'])){
	...
	if(mysqli_num_rows($query) > 0){
		...
		$id_gal=mysqli_query($connect,"UPDATE digi_card SET d_card_status='Active', d_payment_status='Success' WHERE id=".$_POST['card_id']." ");
		if($id_gal){echo '<span class="alert success">Active</div>';}
	}
}			
		
	// activate or deactivate users card from login
if(isset($_POST['fr_id'])){
	$query=mysqli_query($connect,'SELECT * FROM franchisee_login WHERE id="'.$_POST['fr_id'].'" AND f_user_active="YES"');
	
	if(mysqli_num_rows($query) > 0){
		$id_gal=mysqli_query($connect,"UPDATE franchisee_login SET f_user_active='NO' WHERE id=".$_POST['fr_id']." ");
		if($id_gal){echo '<span class="alert danger">NO</div>';}
	}else {
		$id_gal=mysqli_query($connect,"UPDATE franchisee_login SET f_user_active='YES' WHERE id=".$_POST['fr_id']." ");
		if($id_gal){echo '<span class="alert success">YES</div>';}
	}
}	
