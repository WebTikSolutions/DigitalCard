...
// var case with filter
	...
	WHERE
	CASE
    WHEN "'.$filter.'"="All" THEN   d_payment_status LIKE "%"  ELSE  d_payment_status="'.$filter.'"  
	END
...
<!-------------------Pagination-------------------->
		<div class="pagination">
			...
			for($i=1;$i<=$pages;$i++){
				if($_GET['page_no']==$i){
					echo '<a href="index.php?page_no='.$i.'&filter_option='.$filter.'&filter=Submit"><div class="page_btn active">'. $i.'</div></a>';
				}else {
