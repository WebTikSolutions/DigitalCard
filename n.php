...
<head>
        ...
        <!-- Facebook Meta Tags -->
		 <meta property="og:image" content="<?php if(!empty($row['d_logo'])){echo 'https://digicarddemo.site/panel/'.str_replace('../','',$row['d_logo_location']);} ?>">
	...
	<div class="card2" id="shop_online">
		...
		for($x=0;$x<=20;$x++){
			if(!empty($row3["pro_name$x"])){
				...
				//echo "<a href='https://api.whatsapp.com/send?phone=91".str_replace("+91","",$row['d_whatsapp'])."&text=I am interested in Product: ".$row3["pro_name$x"].", Price: ".$row3["pro_mrp$x"]."' target='_blank'><div class='btn_buy'>Enquiry Now</div></a>";
...
if(isset($_POST['email_to_client'])){
        	$to = $row['user_email'];
        $subject = "Customer from ".$_SERVER['HTTP_HOST']."  Name: ".$_POST['c_name'];
        ...
        $headers .= 'From: <info@digicarddemo.site>' . "\r\n";
        $headers .= 'Cc: <info@digicarddemo.site>' . "\r\n";
...
?>	<br>
		...
		<a href="index.php"><div class="create_card_btn"> Create Your Card @ digitalcard.webtiksolutions.com</div></a>
